...
###   This script attempts to download the signature file SHA256SUMS.asc from cocabit.org
...
WORKINGDIR="/tmp/cocabit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://cocabit.org/bin/"
VERSIONPREFIX="cocabit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'cocabit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Cocabit Core binary release signing key installed?"
   fi
